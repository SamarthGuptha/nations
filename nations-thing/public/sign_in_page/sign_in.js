...
while (!rep) {
 ...
 await set(ref(rtdb, 'games/' + thingp), {
    players: [{name: name_2.value + reaper , potato: 4, titanium: 1, uranium: 0,troops: 6}],
    game_running: false,    
...
if (snapshot.exists()) {
    if (!(snapshot.val().players.length  < 6)){
        ...
        update(ref(rtdb, 'games/' + game_code.value), {
            players: [...snapshot.val().players, {name: name.value + reaper, potato: 4, titanium: 1, uranium: 0,troops: 6}],
